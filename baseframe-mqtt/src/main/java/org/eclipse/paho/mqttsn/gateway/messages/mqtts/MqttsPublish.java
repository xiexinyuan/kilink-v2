...
public class MqttsPublish extends MqttsMessage {
    ...
    public byte[] toBytes() {
        ...
        data[delta + 1] = (byte) msgType;
        data[delta + 2] = (byte) flags;
        ...
        if (topicIdType == MqttsMessage.SHORT_TOPIC_NAME)
            ...
            throw new IllegalArgumentException("Unknown topic id type: " + topicIdType);
        System.arraycopy(byteTopicId, 0, data, delta + 3, byteTopicId.length);
        data[delta + 5] = (byte) ((msgId >> 8) & 0xFF);
        data[delta + 6] = (byte) (msgId & 0xFF);
        System.arraycopy(pubData, 0, data, delta + 7, pubData.length);
        return data;
    }
